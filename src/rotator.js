export class Rotator {
  ...
  async incrementAndRotate(linkId) {
    ...
    const row = await this.env.DB.prepare("SELECT verified_count, threshold FROM links WHERE id=?").bind(linkId).first();
    if (!row) { await this.env.DB.exec("ROLLBACK;"); return { ok:false, reason:"link not found" }; }
    const newCount = row.verified_count + 1;
    await this.env.DB.prepare("UPDATE links SET verified_count=?, updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(newCount, linkId).run();
    await this.env.DB.exec("COMMIT;");

    await this.refresh(true);
    await this.env.DB.exec("UPDATE links SET active=0;");
    if (this.cache.activeId) {
      await this.env.DB.prepare("UPDATE links SET active=1 WHERE id=?").bind(this.cache.activeId).run();
    }
